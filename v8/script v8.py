...
with open(filepath) as file:
    ...
    for index, i in enumerate(data):
        ...
        if ('On' in i) and not('v=0' in i) and (index > current_ind) and ('n=' in i):
            ...
            bpy.context.object.color = (0, 0, 0, 1)
            end_time = beat_change_time + bpy.context.scene.render.fps * beat / 0.192 * (tile_loc - beat_change_length/10000) + bpy.context.scene.render.fps * beat / 0.192 * tile_length/10000
            #print (int(end_time))
            ob.keyframe_insert(data_path = "color" , frame = end_time+2)
