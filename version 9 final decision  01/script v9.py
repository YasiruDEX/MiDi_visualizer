...
with open(filepath) as file:
    ...
    for index, i in enumerate(data):
        ...
        if ('On' in i) and not('v=0' in i) and (index > current_ind) and ('n=' in i):
            ...
            if (beats_time > passed_time):
                beat_change_time = beat_change_time + bpy.context.scene.render.fps * (Tempo_time[passed_time + 1]- Tempo_time[passed_time])/100000 * Tempos[passed_time] / 4800;
                end_time = end_time + bpy.context.scene.render.fps * (Tempo_time[passed_time + 1]- Tempo_time[passed_time])/100000 * Tempos[passed_time] / 4800;
                sub = int(Tempo_time[passed_time + 1]- Tempo_time[passed_time]);
                passed_time = index2;
                print("executed " , sub);
            ...
            ob.keyframe_insert(data_path = "color" , frame = beat_change_time + bpy.context.scene.render.fps * beat / 48000 * (tile_loc - beat_change_length/10000)-1)
            ...
            ob.keyframe_insert(data_path = "color" , frame = beat_change_time + bpy.context.scene.render.fps * beat / 48000 * (tile_loc - beat_change_length/10000))
            
            end_time = beat_change_time + bpy.context.scene.render.fps * beat /48000 * (tile_loc - beat_change_length/10000)+1 + bpy.context.scene.render.fps * beat / 48000 * tile_length/10000 + 0
            
            bpy.context.object.color = (1, 1, 1, 1)
            ob.keyframe_insert(data_path = "color" , frame = end_time -1)
