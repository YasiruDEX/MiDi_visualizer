...
with open(filepath) as file:
    ...
    # Splitting data into lines (LOOP FOR NOTES)
...
#------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    Par_pres = False
    for collection in bpy.data.collections:
        if collection.name == "Par":
            Par_pres = True
    if Par_pres == False:         
        bpy.ops.collection.create(name = "Par")
        bpy.context.scene.collection.children.link(bpy.data.collections["Par"])
    
    
    #current index is reset
        current_ind = -1
    
    # Splitting data into lines (LOOP FOR PAR NOTES)
    for index, i in enumerate(data):
        
        
        #finding the next start note
        if ('Par' in i) and ('On' in i) and (index > current_ind):
            # separating the note events into a list
            current_on_item = i.split(' ')
            
            current_ind = index
            
            #finding a par END note line for the current hapening Par stage
            for index, i in enumerate(data):
                if (('Par' in i) and ('v=0' in i)) and (index > current_ind) and (current_on_item[2] in i):
                    # separating the note events into a list
                    current_off_item = i.split(' ')
                    
                    #getting the off index
                    Off_ind = index
                                    
                    #Breaking out of the end Par findign loop
                    break
            
            #finding a par note line for the current hapening Par stage
            for index, i in enumerate(data):
                
                #Breaking out if count passed the Par end tile
                if index < Off_ind:
                    break
                
                #Finding the Par note relevent to the Par start
                if (('On' in i) and ('v=0' in i)) and (index > current_ind) and ('n=' in i) and (current_on_item[2] in i):
                    # separating the note events into a list
                    current_Par_item = i.split(' ')
                    
                    #Code for visualizing the Par tiles
                    ...
                    #getting tiles track
                    temp_String = current_Par_item[3]
                    tile_x = int(temp_String[2::])-60

                    #getting tile length
                    temp_String = current_off_item[0]
                    tile_length = int(temp_String)
                    temp_String = current_Par_item[0]
                    tile_length = tile_length - int(temp_String)
                    ...
                    #giving tiles custom names
                    if tile_channel < 10:
                        note_name = f"ch 0{tile_channel} n {tile_x+60} par"
                    else:
                        note_name = f"ch {tile_channel} n {tile_x+60} par"
                    bpy.context.object.name = note_name
                    ...
                    #scalling the cube
                    bpy.ops.transform.resize(value=(0.5,tile_length/100/2, 0.5), orient_type='GLOBAL', orient_matrix=((1, 0, 0), (0, 1, 0), (0, 0, 1)), orient_matrix_type='GLOBAL', mirror=True, use_proportional_edit=False, proportional_edit_falloff='SMOOTH', proportional_size=1, use_proportional_connected=False, use_proportional_projected=False)
