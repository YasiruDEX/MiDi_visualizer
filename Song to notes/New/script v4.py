...
filepath = 'C:/Users/Techtooth/Desktop/Song to notes/new/coler keyframe on.txt'
...
with open(filepath) as file:
    ...
    #creating empty for cube animation
    if not("empty_for animation" in bpy.data.objects):
        bpy.ops.object.empty_add(type='PLAIN_AXES', align='WORLD', location=(0, 0, 0), scale=(1, 1, 1))
        bpy.context.object.name = "empty_for animation"
        bpy.ops.object.move_to_collection(collection_index=1)
    
    an_empty = bpy.data.objects["empty_for animation"]
    
    #creating the collection for Notes
    Notes_pres = False
    ...
    for index, i in enumerate(data):
        ...
        if ('On' in i) and not('v=0' in i) and (index > current_ind) and ('n=' in i):
            ...
            #assigning Parent
            bpy.context.object.parent = an_empty
